...
.PHONY: help proto install dev test lint format clean docker run ci
...
proto: ## Regenerate Python gRPC stubs from proto/ext_mcp.proto
	$(PY) -m grpc_tools.protoc -I proto \
		...
		proto/ext_mcp.proto
	@echo "stubs regenerated in proto/"
...
lint: ## Lint with ruff
	$(PY) -m ruff check guardrails server.py tests

format: ## Format with ruff
	$(PY) -m ruff format guardrails server.py tests
	$(PY) -m ruff check --fix guardrails server.py tests
...
ci: lint test ## What CI runs on every PR
